...
plt = twoDPlot(xi[0:2],np.zeros(2),plotstyle='plot',label='the 1st beam element',c='green')
plt = twoDPlot(xi[1:3],np.zeros(2),plotstyle='plot',label='the 2nd beam element',c='red')
plt = twoDPlot(xi[0:3],np.zeros(3),plotstyle='scatter',label='first three beam nodes',c='black')
plt = twoDPlot([xi[1]]*2,[yBot[divide_list['BOT']][1],yTop[divide_list['TOP']][1]],c='grey',alpha=0.5,linestyle='dashed')
plt.xlim([0,xi[2]])
plt.ylabel('y')
finalizePlot(plt)
#finalizePlot(plt,title='An example of two beam elements',savefig=True,fname='beam_example_element.eps')