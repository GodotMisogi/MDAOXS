...
plt.plot(airfoil.boundaryPoints_X, prob['input.y'],'-.',label='the original vortex panel')
plt.plot(airfoil.boundaryPoints_X, airfoil.boundaryPoints_Y, '-x',label='the changed vortex panel')
plt.title(s='Result of using vertex panel method with %d panels'%airfoil.NUM_SAMPLES)
plt.legend()
plt.xlabel('x')
plt.ylabel('y')
plt.savefig(fname='vpm_result.eps')
print(np.array(prob['input.y'])-np.array(airfoil.boundaryPoints_Y))

from util.plot import *
plt.figure()
#plt = interpolatePlot(airfoil.boundaryPoints_X,airfoil.boundaryPoints_Y)
x_bot = np.array(airfoil.boundaryPoints_X[0:int(airfoil.NUM_SAMPLES/2+1)])
x_top = np.array([0]+airfoil.boundaryPoints_X[int(airfoil.NUM_SAMPLES/2+1):])
y_bot = np.array(airfoil.boundaryPoints_Y[0:int(airfoil.NUM_SAMPLES/2+1)])
y_top = np.array([0]+airfoil.boundaryPoints_Y[int(airfoil.NUM_SAMPLES/2+1):])

plt = interpolatePlot(x_top,y_top,plotstyple='scatter',N=396,label='rebuilt shape',c='orange',s=0.2)
plt = interpolatePlot(x_bot[::-1],y_bot[::-1],plotstyple='scatter',N=396,c='orange',s=0.2)

#plt = interpolatePlot(x_bot,y_bot,plotstyple='scatter',N=396,c='orange',s=0.2)
...
finalizePlot(plt,title='The result of the vortex panel model with %d panels'%(airfoil.NUM_SAMPLES),savefig=True,fname='vpm_result_rebult.eps')